...
CXXINC=-I$(ARIA)/include -I$(ARIA)/ArNetworking/include -I$(ARIA)/ArVideo/include -I$(FREENECT2_DIR)/include -I$(FREENECT2_DIR)/src/tinythread
...
kinectArVideoServer: kinectArVideoServer.cpp ArVideoOpenCV.h
	$(CXX) $(CXXFLAGS) $(CXXINC) $< -o $@ $(CXXLINK_FREENECT) $(CXXLINK_ARVIDEO)

testOpenCVServer: testOpenCVServer.cpp ArVideoOpenCV.h 
	$(CXX) $(CXXFLAGS) $(CXXINC) $< -o $@ $(CXXLINK_OPENCV) $(CXXLINK_ARVIDEO)

testRemoteForwarder: testRemoteForwarder.cpp  ArVideoOpenCV.h  
